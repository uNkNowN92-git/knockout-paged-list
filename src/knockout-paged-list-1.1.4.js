var PagedList = (function (ko) {
    return function (option) {
        ...
        self.loadedEntriesCount = ko.pureComputed(function () {
            var data = self.data();
            var __data = typeof (data) === "function" ? data() : data;

            return NotEmptyItemsCount(__data);
        });
        ...
        self.getList = function (data, event) {
            if (_isPreserveCurrentPage === true) {
                _requestedPage(self.currentPage());
            } else {
                _requestedPage(1);
            }
        ...
        function MapData() {
            ...
            if (_dataAsObservable === false) {
                ...
                mappedData = _currentData; //ko.mapping.fromJS(_currentData, _mappingNotAsObservable);
        ...
        function GetDataUrl(event) {
            var url = event !== undefined ? $(event.target).data("url") : _defaultUrl;
            self.setUrl(url);
        }
